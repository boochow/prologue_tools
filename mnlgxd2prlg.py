...
converted = os.path.splitext(args[0])[0] + ".prlgprog"
if write_file:
...
if delay_is_on:
    ...
    val = int.from_bytes(rawdata[151:153], byteorder='little') - 1
    if val != 511:
        newdata[57:59] = int.to_bytes(val, 2, byteorder='little')
elif reverb_is_on:
    newdata[62] = 2
    newdata[63:65] = rawdata[107:109]
    newdata[65:67] = rawdata[109:111]
    val = int.from_bytes(rawdata[153:155], byteorder='little') - 1
    if val != 511:
        newdata[57:59] = int.to_bytes(val, 2, byteorder='little')
